...
if __name__ == "__main__":
    # ROC-AUC metric:	train=0.7167, test=0.68814
    # Accuracy metric:	train=0.71671, test=0.68813
    # F1_score metric:	train=0.71954, test=0.68742
    # Precision metric:	train=0.713, test=0.68568
    # Recall metric:	train=0.72621, test=0.68917
    TRAIN = True
    OUTPUT_SIZE = 50000
    ...
    if TRAIN:
        ...
        data = binary_balance(
                data=read_csv("datasets/process_spam_best.csv"),
                target="target",
        ...
        if test:
            ...
            save('spam.npy', text_embeddings)
            
        else:
            text_embeddings = load('spam.npy')

        x_train, x_test, y_train, y_test = train_test_split(
            text_embeddings, target, train_size=0.85, random_state=42,
        )

        is_useful = IsUsefulFromSpam()
